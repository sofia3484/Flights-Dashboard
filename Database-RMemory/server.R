server <- function(input, output, session) {
  ...
  showNotification("Created by Sofia Nicklaus S.", duration = NULL, type = "message")
  ...
  base_flights <- reactive({
    res <- flights %>%
      filter(carrier == local(input$airport)) %>%
  ...
  output$total_flights <- renderValueBox({
    ...
    base_flights() %>%
      ...
      as.integer() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("chart-line"), color = "teal",subtitle = "Number of Flights")
  })
  ...
  output$per_day <- renderValueBox({
    ...
    base_flights() %>%
      ...
      round() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("equals"), color = "olive",subtitle = "Average Flights per day")
  })
  ...
  output$percent_delayed <- renderValueBox({
    base_flights() %>%
      ...
      round() %>%
      paste0("%") %>%
      valueBox(icon = icon("tachometer-alt"), color = "aqua", subtitle = "Flights delayed")
  })
